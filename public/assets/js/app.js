...
function cart() {
    let keranjang = JSON.parse(localStorage.getItem("chart"));
    $("#e").empty();
    $.each(keranjang, (i, val) => {
        $("#e").append(
            `<div class="d-block dropdown-item dropdown-item-cart text-wrap px-3 py-2">
                ...
                .replace(/\B(?=(\d{3})+(?!\d))/g, ".")} </span>
                                                ...
                                                <div class="ps-2">
                                                    <button type="button"
                                                        class="btn btn-icon btn-sm btn-ghost-secondary remove-item-btn" data-uuid="${val.id}" id="delcart"><i
